# Callx Voice , Build Contract (single source of truth for the swarm)
...
**Callx Voice**. The hook: production voice agents at roughly one rupee, powered by
...
- On boot: if `data/db.json` missing, create it and seed a demo tenant + owner
  (email `demo@callx.ai`, password `callxvoice`, tenant `Callx Demo`) and migrate any
