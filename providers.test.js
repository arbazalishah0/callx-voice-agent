...
test('VoBiz status and dial accept 64-bit integer phone number IDs like 30495596953997960', async () => {
  resetEnv({
    DOGRAH_BASE_URL: 'https://app.dograh.com',
    DOGRAH_API_KEY: 'test-key',
    DOGRAH_WORKFLOW_ID: 'afd98484-b2c9-493a-b2b7-78d8252efb54',
    DOGRAH_TELEPHONY_CONFIG_ID: '1732',
    DOGRAH_PHONE_NUMBER_ID: '30495596953997960',
    VOBIZ_NUMBER: '+911171366938',
  });
  const requests = [];
  const mockHttp = async (host, path, headers, body) => {
    requests.push({ host, path, headers, body: body ? JSON.parse(body.toString('utf8')) : null });
    if (path.includes('/phone-numbers')) {
      return {
        status: 200, headers: {},
        buffer: Buffer.from(JSON.stringify({
          phone_numbers: [{ id: 30495596953997960, address: '+911171366938', is_active: true }],
        })),
      };
    }
    if (path.includes('/telephony-configs')) {
      return {
        status: 200, headers: {},
        buffer: Buffer.from(JSON.stringify({
          configurations: [{ id: 1732, name: 'VoBiz Outbound', provider: 'vobiz', is_default_outbound: true }],
        })),
      };
    }
    if (path.includes('/initiate-call')) {
      return { status: 200, headers: {}, buffer: Buffer.from(JSON.stringify({ call_id: 'c1' })) };
    }
    return { status: 404, headers: {}, buffer: Buffer.from('{}') };
  };
  const providers = loadProviders(mockHttp, mockHttp);

  const status = await providers.telephony.status();
  assert.equal(status.connected, true);
  assert.equal(status.did, '+911171366938');

  const dialResult = await providers.telephony.dial('9876543210');
  assert.equal(dialResult.status, 200);
  const initiateReq = requests.find((r) => r.path.includes('/initiate-call'));
  assert.equal(initiateReq.body.from_phone_number_id, 30495596953997960);
  assert.equal(initiateReq.body.phone_number, '+919876543210');

  // Test international and various format normalizations
  assert.equal(providers.normalizeDialNumber('9876543210'), '+919876543210');
  assert.equal(providers.normalizeDialNumber('09876543210'), '+919876543210');
  assert.equal(providers.normalizeDialNumber('+91 98765 43210'), '+919876543210');
  assert.equal(providers.normalizeDialNumber('+14155552671'), '+14155552671');
  assert.equal(providers.normalizeDialNumber('+447911123456'), '+447911123456');

  const intlResult = await providers.telephony.dial('+14155552671');
  assert.equal(intlResult.status, 200);
  const intlReq = requests.filter((r) => r.path.includes('/initiate-call')).pop();
  assert.equal(intlReq.body.phone_number, '+14155552671');
});
