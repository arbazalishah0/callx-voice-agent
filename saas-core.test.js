...
test('schema migration is additive and normalizes roles', () => {
  ...
  for (const key of [
    'wallets', 'ledger', 'paymentIntents', 'paymentEvents', 'supportTickets',
    'supportMessages', 'auditEvents', 'presets', 'byonConnections', 'hvacJobs',
    'hvacSettings', 'demoLinks', 'invoices', 'invoiceEvents', 'integrationRequests',
    'agencyPrompts', 'clientActivities', 'tenantStatusEvents',
    'customers', 'customerMemories', 'calls', 'campaigns', 'workflows',
    'knowledgeBase', 'phoneNumbers', 'followUps'
  ]) assert.ok(Array.isArray(migrated[key]));
});
