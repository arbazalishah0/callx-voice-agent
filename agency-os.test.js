...
const ADMIN_EMAIL = 'agency.qa@callxai.com';
const ADMIN_PASSWORD = 'Agency-QA-Only-2026!';
const CLIENT_EMAIL = 'owner.qa@callxai.com';
...
test('agency OS APIs complete the lifecycle and preserve tenant isolation', { timeout: 30000 }, async (t) => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'callx-agency-os-'));
  ...
  assert.equal(admin.tenant.name, 'Callx Agency QA');
